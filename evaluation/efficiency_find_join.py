    ...
    for join_method in [
        ...
        for o_t in overlaps:
            ...
            for st_schema in tqdm(sorted_st_schemas):
                ...
                _end = time.time()
                tbl_perf_profiles[agg_name] = (_end - _start, method.value)
            ...
            dump_json(
                f"evaluation/run_time/{data_source}/{input}/perf_time_{granu_list[0]}_{granu_list[1]}_{join_method.value}_{o_t}.json",
                corr_search.perf_profile,
            )
            dump_json(
                f"evaluation/run_time/{data_source}/{input}/perf_time_per_tbl_{granu_list[0]}_{granu_list[1]}_{join_method.value}_{o_t}.json",
                tbl_perf_profiles,
            )
